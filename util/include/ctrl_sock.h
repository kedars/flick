 ...
 * fd_set in select()
 * \param[in] port the local port on which the control socket will
 * listen
 *
 * \return the socket descriptor that can be added to the fd_set in
 * select.
 * \return an error code if less than zero
 */
int cs_create_ctrl_sock(int port);

/** Free the control socket
 *
 * This frees up the control socket that was earlier created using
 * cs_create_ctrl_sock()
 *
 * \param[in] fd the socket descriptor associated with this control
 * socket
 */
 ...
 * \param[in] data pointer to a buffer that contains data to send on
 * the socket
 * \param[in] data_len the length of the data contained in the buffer
 * pointed to be data
 *
 * \return the number of bytes sent to the control socket
 * \return an error code if less than zero
 */
int cs_send_to_ctrl_sock(int port, void *data, unsigned int data_len);
